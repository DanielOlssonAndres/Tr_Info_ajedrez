...
void Tablero::Tomar_Pieza_VSMAQ(Vector2xy origen) {
	...
	if (origen.x != -1 && origen.y != -1 && matriz[origen.x][origen.y] != 0) { //Si hemos seleccionado una casilla dentro del tablero
		...
		if (pInd != -1) {
			if ((color && fichas[pInd]->Get_Valor() < 0) || (!color && fichas[pInd]->Get_Valor() > 0) || fichas[pInd]->Get_Valor() > 0) { //Si la pieza no corresponde con el color del turno
				pInd = -1;
				std::cout << "No puedes mover esa ficha, es el turno de la maquina" << std::endl;
				ETSIDI::play("sonidos/SonidoError.wav");
			}
			if (pInd != -1) {
				pI = origen.x;
					...
					{//RECORRE MATRIZ PARA SELECCIONAR FICHA DE SU COLOR
						if (matriz[i][j] > 0) {
							if (rand() % 10 == 5) {
								...
								for (int k = 0; k < 6; k++) {
									for (int l = 0; l < 5; l++) {
										if (Selec_Mover(k, l, elige_jugada) = true)
										{
											mover = true;
											if (rand() % 5 == 3) {
												if (Selec_Mover) {
													bool flag_mover = true;
													return flag_mover;
												}
											}
										}
